...
"""
Script for training a model to predict properties in the photoswitch dataset using Gaussian Process Regression with SOAP Kernel.
"""
...
from data_utils import transform_data, TaskDataLoader
from SOAP_GP.create_kernel import generate_kernel
from kernels import Soap

def main(path, task,  use_pca, n_trials, test_set_size, use_rmse_conf):
    """
    ...
    # Generates SOAP kernel if it has not been generated already

    try:
       K_mat = np.load('SOAP_GP/tmp/'+task+'.npy')
    except:
       K_mat = generate_kernel(task, smiles_list)
       np.save('SOAP_GP/tmp/'+task+'.npy',K_mat)

    # We define the Gaussian Process Regression Model using the Soap kernel
    ...
    if use_rmse_conf:
        ...
        plt.savefig(task + '/results/soapgp/confidence_curve_rmse.png')
        plt.show()
        ...
        plt.xlim([0, 100 * ((len(y_test) - 1) / len(y_test))])
        plt.yticks(np.arange(0, np.max(upper) + 1, 5.0))
        plt.savefig(task + '/results/soapgp/confidence_curve_mae.png')
        plt.show()
...
if __name__ == '__main__':
    ...
    main(args.path, args.task, args.use_pca, args.n_trials, args.test_set_size, args.use_rmse_conf)
